...
import type { Good, MarketOrder, Port } from "@/lib/types";
...
export default function MarketPage() {
  ...
  useEffect(() => {
    Promise.all([worldApi.getPorts(), worldApi.getGoods()])
      .then(([p, g]) => { setPorts(p); setGoods(g); })
      .catch(console.error)
  ...
  const handleFill = async (order: MarketOrder) => {
    ...
    try {
      await marketApi.fillOrder(order.id, { quantity: Number(fillQty) });
